...
## Layout comprises two tabs - one for viewing of the graph and the other for making a music list
...
app.layout = html.Div(
    [
        html.Div(
            className="row",
            children=[html.H1(children="Music Graph Network")],
            style={
                "textAlign": "center",
                "backgroundColor": "#1a1a1a",
                "color": "white",
        ...
        html.Div(
            [
                ...
                dcc.Tabs(
                    ...
                    children=[
                        dcc.Tab(
                            label="Explore Network Graph",
                            value="tab-1",
                            style=tab_style,
                            selected_style=tab_selected_style,
                        ),
                        dcc.Tab(
                            label="Music List Builder",
                            value="tab-2",
                            style=tab_style,
                            selected_style=tab_selected_style,
                        ),
            ...
            html.Div(
                ...
                children=[
                    html.Div(
                        className="col",
                        children=[
                            html.H4("Select a graph"),
                            # html.P("Select a graph, named according to basket size"),
                            dd_segment,
                        ],
                ...
                children=[
                    html.Div(
                        children=[
                            dcc.Loading(
                                id="loading-icon",
                                children=[dcc.Graph(id="graph-graphic")],
                            )
                        ]
                    )
                ],
            ...
            html.Div(
                ...
                children=[
                    html.Div(
                        ...
                        children=[
                            html.P(
                                "Description: Search for items to start building your music. \
                ...
                children=[
                    html.Div(
                        ...
                        children=[
                            html.Div(
                                ...
                                children=[
                                    ...
                                    dcc.Input(
                                        id="item_search",
                                        type="search",
                                        placeholder="Search songs,users or artists ",
                                        debounce=True,
                                        value="trackb9fa88d18f7dda845789155d69177df0",
                                    ),
                                    html.P(""),
                                    ...
                                    html.P(
                                        id="explainer",
                                        children=[
                                            "The items below are the closest that match your search"
                                        ],
                            ...
                            # recommended items
                            html.Div(
                                className="row", id="button-container", children=[]
                            ),
                        ],
                    ),
                    html.Div(
                        ...
                        children=[
                            html.Div(
                                ...
                                children=[
                                    html.Div(
                                        className="six columns",
                                        children=[html.H3("Your Music List")],
                                    )
                                ],
                            ),
                            # Dyanamic Music list is built here
                            html.Div(
                                className="row",
...
@app.callback(
    ...
    Output("music-list-container", "children"),
    ...
    [
        ...
        State("music-list-container", "children"),
        State("explainer", "children"),
    ],
)
def display_search_buttons(
    item, vals, sim_val, buttons, music_list_items, explainer_text
):
    """Function that runs all of the updates on for the music list"""
    ...
    if (
        ctx.triggered is not None
        and ctx.triggered[0]["prop_id"] == "."
        or ctx.triggered[0]["prop_id"] == "item_search.value"
    ):
        buttons = []
        # Search for music items based on the item typed in the search bar
        # and create buttons
        music_items = find_similar_songs(nodes, item)
        # Sort the items by length to make the display look nicer
        music_items.sort(key=len)
        counter = 0
        for i, it in enumerate(music_items):
            new_button = html.Button(
                f"{it}",
                id={"type": "button", "index": it},
                n_clicks=0,
                style=button_style,
            )
            counter += 1
            ...
            if counter > 30:
                break
            explainer_text = (
                r"""The items below are the closest that match your search"""
            )
    ...
    # & it's not the search input being searched in and it's not the radio button being checked
        ...
        ctx.triggered
        and ctx.triggered[0]["value"] != 0
        and ctx.triggered[0]["value"] is not None
        and ctx.triggered[0]["prop_id"] != "item_search.value"
        and ctx.triggered[0]["prop_id"] != "sim-radio.value"
    ):
        ...
        # Add it to the music list
        new_item = html.P(f"{button_clicked}")
        music_list_items.append(new_item)

        # Erase the list of items and present similar items by searching the graph
        buttons, explainer_text = recommend_music(button_clicked, sim_val)
    ...
    return buttons, music_list_items, explainer_text


def recommend_music(button_clicked, sim_val):
    """Function that returns a list of recommended music
    ...
    explainer_text = (
        r"""These items are recommended based on the last item added to your list"""
    )
    ...
    for i, it in enumerate(recommendations):
        new_button = html.Button(
            f"{it}", id={"type": "button", "index": it}, n_clicks=0, style=button_style
        )
        buttons.append(new_button)
...
# they're looking at
@app.callback(Output("graph-graphic", "figure"), [Input("dd_segment", "value")])
