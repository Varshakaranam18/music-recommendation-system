...
def get_neighbours(G, item, topn=10):
    ...
    # TODO: Find and fix a issue here
    for nodes in list(G.edges(str(item))):
        weights[nodes[1]] = G.get_edge_data(nodes[0], nodes[1])["weight"]
    weights_sorted = {
        k: v for k, v in sorted(weights.items(), key=lambda x: x[1], reverse=True)
    }
...
def find_similar_songs(nodes, ingredient="Pear"):
    """Function that returns the closet match to an node in the graph
    ...
    for node in nodes:
        # This does a string-like search for the item in each node
